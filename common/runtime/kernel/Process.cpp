...
namespace runtime {
    ...
    Process::~Process() {
        ...
        // Возвращаем память в пул если нужно
        if (pool && heap_base) {
            // В зависимости от типа пула может потребоваться
            // специальная логика возврата памяти
        }
    }
    ...
    void Process::dump_info() const {
        ...
        lg::debug("  Mask: {:08x}", static_cast<u32>(mask));
    ...
    void Process::update_state_hooks() {
        if (current_state) {
            ...
            event_hook = current_state->event_bytecode;
        }
        else {
            trans_hook = nullptr;
            post_hook = nullptr;
