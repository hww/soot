    ...
    for (auto it = bfs_order.rbegin(); it != bfs_order.rend(); it++) {
        ...
        case Node::Kind::LIST: {
            if (node->break_list) {
                ...
                int nodes_on_first_line = std::min(static_cast<int>(node->child_nodes.size()),
                ...
                for (size_t node_idx = nodes_on_first_line; node_idx < node->child_nodes.size();
                     node_idx++) {
...
void insert_required_breaks(const std::vector<Node *> &bfs_order) {
    const std::unordered_set<std::string> always_break = {
        ...
        "rlet",  "defstate",    "behavior",  "loop",    "let*"};

    for (auto node : bfs_order) {
        if (!node->break_list && node->kind == Node::Kind::LIST && node->child_nodes.size() > 0 &&
            node->child_nodes[0].kind == Node::Kind::ATOM) {
...
int run_algorithm(const std::vector<Node *> &bfs_order, int line_length) {
    ...
    for (auto it = bfs_order.rbegin(); it != bfs_order.rend(); it++) {
        ...
        if (node->kind != Node::Kind::ATOM && static_cast<int>(node->text_len) > line_length &&
            !node->break_list) {
            break_list(node);
            num_broken++;
            if (!min_depth) {
...
void append_node_to_string(const Node *node, std::string &str, int init_indent_level,
    ...
    case Node::Kind::LIST:
        if (node->break_list) {
            ...
            int listing_indent =
                next_indent_level + node->get_quote_length() + node->sub_elt_indent;
            int extra_indent = 0;
            ...
            for (; node_idx < node->top_line_count && node_idx < node->child_nodes.size();
                 node_idx++) {
