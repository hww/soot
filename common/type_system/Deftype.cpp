...
void add_field(StructureType *structure, TypeSystem *ts, const script::Object &def,
    ...
    if (!rest->is_null()) {
        if (car(rest).is_integer()) {
            ...
            rest = cdr(rest);
        } else if (car(rest).is_symbol() && constants &&
                   constants->lookup((car(rest)).as_symbol())) {
                ...
                if (param.is_symbol()) {
                    ...
                    while (!name_it->is_null()) {
                        ...
                        if (deref_field.is_integer()) {
                            auto ref_array_field =
                                !type_to_use && !overlay_field.is_inline()
                                    ? ts->lookup_type_allow_partial_def(overlay_field.type())
                                    : nullptr;
                            if (ref_array_field) {
                                // we can have an array of references (non-inline) to a
                                // forward-declared type
                                offset_override +=
                                    ref_array_field->get_load_size() * deref_field.as_integer();
                            } else {
                                ...
                                if (!deref_info.can_deref) {
                                    throw std::runtime_error(fmt::format(
                                        "Array could not be dereferenced for overlay-at in {}",
                                        name));
                                }
                                // overlay_field.type() = deref_info.result_type;
                                offset_override += deref_info.stride * deref_field.as_integer();
                            }
                        } else {
                            if (!type_to_use) {
                                throw std::runtime_error(fmt::format(
                                    "Field {} not inside a structure for overlay-at in {}",
                                    overlay_field.name(), name));
                            }
                            auto field_name = symbol_string(car(name_it));
                            if (!type_to_use->lookup_field(field_name, &overlay_field)) {
                                throw std::runtime_error(fmt::format(
                                    "Field {} not found to overlay for {}", field_name, name));
                            }
                            type_to_use = dynamic_cast<StructureType *>(
                                ts->lookup_type_no_throw(overlay_field.type()));
                            offset_override += overlay_field.offset();
                        }
                        name_it = cdr(name_it);
                    }
                    ...
                    throw std::runtime_error(
                        fmt::format("Unknown parameter for overlay-at in {}", name));
                }
...
struct StructureDefResult {
    ...
    void append_virtual_state_def(const std::string &state_name, const StateHandler handler,
                                  DefinitionMetadata data) {
        if (virtual_state_definitions.count(state_name) == 0) {
            virtual_state_definitions[state_name] =
                std::unordered_map<std::string, DefinitionMetadata>();
        }
        virtual_state_definitions[state_name][handler_kind_to_name(handler)] = data;
    }

    void append_state_def(const std::string &state_name, const StateHandler handler,
                          DefinitionMetadata data) {
...
StateHandler handler_keyword_to_kind(const std::string &name) {
    if (name == ":enter") {
        ...
        throw std::runtime_error("Unknown handler keyword: " + name);
    }
}
        ...
        if (!overriding_doc) {
            ...
            while (!obj->is_null() && car(obj).is_keyword()) {
                ...
                if (keyword.name_ptr == ":no-virtual") {
                    no_virtual = true;
                } else if (keyword.name_ptr == ":replace") {
                    replace_method = true;
                    ...
                    if (!cdr(obj)->is_null() && car(cdr(obj)).is_list()) {
                        ...
                        while (!elem->is_null() && car(elem).is_symbol()) {
                            ...
                            if (!car(elem).is_string()) {
                                throw std::runtime_error(
                                    "Missing a docstring for a state handler!");
                            }
                            DefinitionMetadata def_meta;
                            // TODO - definition location info
                            def_meta.docstring = car(elem).as_string()->data;
                            struct_def.append_virtual_state_def(method_name, handler_kind,
                                                                def_meta);

                            elem = cdr(elem);
                        }
                    }
                } else if (keyword.name_ptr == ":behavior") {
                    obj = cdr(obj);
                ...
                } else if (keyword.name_ptr == ":overlay-at") {
                    obj = cdr(obj);
                    if (!car(obj).is_symbol()) {
            ...
            if (it.is_symbol() && it.as_symbol().name_ptr[0] == ':') {
                ...
                if (std::string_view(tag_name) == "behavior") {
                    if (!type_system->fully_defined_type_exists(tag_val.as_symbol().name_ptr) &&
                        ...
                        throw std::runtime_error(fmt::format("Behavior tag uses an unknown type {}",
                                                             tag_val.as_symbol().name_ptr));
                    }
...
DeftypeResult parse_deftype(const script::Object &deftype, TypeSystem *ts,
    ...
    try {
        ...
        if (is_type("basic", parent_type, ts)) {
            ...
            if (pto->final()) {
                throw std::runtime_error(
                    fmt::format("[TypeSystem] Cannot make a child type {} of final basic type {}",
            ...
            if (sr.always_stack_singleton) {
                lg::print("[TypeSystem] :always-stack-singleton was set on {}, which is a basic "
                          "and cannot "
            ...
            auto new_type =
                std::make_unique<StructureType>(parent_type_name, name, false, false, false, 0);
            new_type->m_metadata = symbol_metadata;
            ...
            if (sr.final) {
                throw std::runtime_error(fmt::format(
                    "[TypeSystem] :final option cannot be used on structure type {}", name));
            }
            new_type->set_heap_base(result.flags.heap_base);
            ts->add_type(name, std::move(new_type));
        ...
        if (structure_result) {
            ...
            result.type_info->m_virtual_state_definition_meta =
                structure_result->virtual_state_definitions;
        }
        return result;
