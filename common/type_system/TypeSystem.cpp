...
Type *TypeSystem::add_type(const std::string &name, std::unique_ptr<Type> type) {
    ...
    if (method_kv != m_forward_declared_method_counts.end()) {
        ...
        if (method_count != method_kv->second) {
            throw_typesystem_error(
                "Type {} was defined with {} methods, but was forward declared with {}", name,
    ...
    if (kv != m_types.end()) {
        ...
        if (*kv->second != *type) {
            ...
            if (dynamic_cast<NullType *>(type.get()) &&
                dynamic_cast<NullType *>(kv->second.get())) {
        ...
        if (name != "object" && name != "none" && name != "_type_" && name != "_varargs_") {
            ...
            auto fwd_parent_it = m_forward_declared_types.find(type->parent());
            if (fwd_parent_it != m_forward_declared_types.end()) {
                throw_typesystem_error(
                    "Cannot create new type {}. The parent type {} is not fully defined.", name,
                    type->parent());
            }

            auto parent_it = m_types.find(type->parent());
            if (parent_it == m_types.end()) {
                // ДОБАВЛЕНО: Для bitfield разрешаем создание без родителя
                if (type->parent() != "bitfield") {
                    throw_typesystem_error(
                        "Cannot create new type {}. The parent type {} is not defined.", name,
                        type->parent());
                }
            }
        }
        ...
        if (fwd_it != m_forward_declared_types.end()) {
            if (!tc(TypeSpec(fwd_it->second), TypeSpec(name))) {
                throw_typesystem_error(
                    "Type {} was originally declared as a child of {}, but is not.", name,
                    fwd_it->second);
            }
        }
...
bool TypeSystem::typecheck_base_types(const std::string &expected, const std::string &actual,
    ...
    // Handle unit types
    if (exp == "meters" || exp == "degrees")
        exp = "float";
    if (act == "meters" || act == "degrees")
        act = "float";
    ...
    if (exp == act || exp == lookup_type_allow_partial_def(act)->name()) {
        return true;
    }
    ...
    while (current_type->has_parent()) {
        current_actual = current_type->parent();
    ...
    if (override_type) {
        if (!got_existing) {
            throw_typesystem_error(
                ...
                "previously declared in a parent",
                method_name, type->name());
        }

        return type->add_method({existing_info.id, method_name, ts, type->name(),
                                 type->name(), no_virtual, true, false, docstring,
                                 std::nullopt});
    } else {
        if (got_existing) {
            // Verify compatibility
            if (!existing_info.type.is_compatible_child_method(ts, type->name())) {
                throw_typesystem_error(
                    "The method {} of type {} was originally declared as {}, but has been "
                    "redeclared as {}. Originally declared in {}",
                    method_name, type->name(), existing_info.type.print(), ts.print(),
            ...
            // Add new method
            return type->add_method({get_next_method_id(type), method_name, ts, type->name(),
                                     type->name(), no_virtual, false, false, docstring,
                                     std::nullopt});
        }
    }
}
...
MethodInfo TypeSystem::add_new_method(Type *type, const TypeSpec &ts,
    ...
    if (type->get_my_new_method(&existing)) {
        // Verify compatibility
        if (!existing.type.is_compatible_child_method(ts, type->name())) {
            throw_typesystem_error(
                ...
                type->name(), existing.type.print(), ts.print());
        }
        return existing;
    } else {
        return type->add_new_method({0, "new", ts, type->name(), type->name(), false, false,
...
MethodInfo TypeSystem::define_method(Type *type, const std::string &method_name, const TypeSpec &ts,
    ...
    if (got_existing) {
        ...
        int bad_arg_idx = -1;
        if (!existing_info.type.is_compatible_child_method(ts, type->name(), &bad_arg_idx)) {
            if (bad_arg_idx == -1) {
                throw_typesystem_error(
...
MethodInfo TypeSystem::overlay_method(Type *type, const std::string &method_name,
    ...
    if (!got_existing) {
        throw_typesystem_error(
            ...
            method_overlay_name, type->name());
    }

    // CORRECTED: Proper construction
    return type->add_method({existing_info.id, method_name, ts, type->name(), type->name(),
...
MethodInfo TypeSystem::override_method(Type *type, const std::string &method_name,
    ...
    MethodInfo existing_info;
    bool       exists = try_lookup_method(type->parent(), method_name, &existing_info);
    if (!exists) {
    ...
    return type->add_method({existing_info.id, method_name, existing_info.type, type->name(),
                             type->name(), existing_info.no_virtual,
...
int TypeSystem::add_field_to_type(StructureType *type, const std::string &field_name,
                                  const TypeSpec &field_type, bool is_inline, bool is_dynamic,
                                  int array_size, int offset_override, bool skip_in_static_decomp,
                                  double score, const std::optional<TypeSpec> decomp_as_ts) {

    // Проверяем существование поля
    Field existing_field;
    if (type->lookup_field(field_name, &existing_field)) {
        throw_typesystem_error("Type {} already has a field named {}", type->name(),
                               field_name);
    }
    ...
    if (Type::verbose)
        fmt::print(
            "DEBUG: Added field {} to type {}, offset: {}, size: {}, inline: {}, array: {}\n",
            field_name, type->name(), offset, field_size, is_inline, array_size);
...
void TypeSystem::add_builtin_types() {
    ...
    declare_method(obj_type, "inspect", {}, false, make_function_typespec({"_type_"}, "_type_"),
                   false);
    declare_method(obj_type, "length", {}, false, make_function_typespec({"_type_"}, "int"), false);
...
void TypeSystem::add_field_to_bitfield(BitFieldType *type, const std::string &field_name,
    ...
    if (field_size > load_size) {
        throw_typesystem_error(
            ...
            type->name(), field_name, field_size, load_size);
    }

    if (field_size + offset > type->get_load_size() * 8) {
        throw_typesystem_error("Type {}'s bitfield {} will run off the end of the type (ends at {} "
                               "bits, type is {} bits)",
                               type->name(), field_name, field_size + offset,
                               type->get_load_size() * 8);
    }
...
std::string TypeSystem::generate_deftype_footer(const Type *type) const {
    ...
    if (new_info) {
        ...
        methods_string.append(fmt::format(") {})", new_info->type.last_arg().print()));
...
bool TypeSystem::should_use_virtual_methods(const Type *type, int method_id) const {
    ...
    if (as_basic && !as_basic->final()) {
        auto method_info = lookup_method(type->name(), method_id);
...
bool TypeSystem::should_use_virtual_methods(const TypeSpec &type, int method_id) const {
    ...
    if (it != m_types.end()) {
        ...
        if (fwd_dec_type->name() == "structure") {
...
TypeSystem::search_types_by_parent_type_strict(const std::string &parent_type) {

    std::vector<std::string> results;
    for (const auto &[type_name, type_info] : m_types) {
        if (type_info->has_parent() && type_info->parent() == parent_type) {
            results.push_back(type_name);
        }
    }
    return results;
...
std::vector<std::string> TypeSystem::search_types_by_minimum_method_id(
    const int minimum_method_id, const std::optional<std::vector<std::string>> &existing_matches) {
...
std::vector<std::string>
TypeSystem::search_types_by_size(const int min_size, const std::optional<int> max_size,
                                 const std::optional<std::vector<std::string>> &existing_matches) {
...
void TypeSystem::forward_declare_type_method_count(const std::string &name, int num_methods) {
    ...
    if (existing_type != m_types.end()) {
        ...
        if (existing_count != num_methods) {
            throw_typesystem_error("Type {} was defined with {} methods and is now being forward "
                                   "declared with {} methods",
...
int TypeSystem::get_type_method_count(const std::string &name) const {
    ...
    throw_typesystem_error(
        "Tried to find the number of methods on type `{}`, but it is not defined.", name);
    return -1;
}
...
void TypeSystem::assert_field_offset(const std::string &type_name, const std::string &field_name,
                                     int offset) {
    Field field = lookup_field(type_name, field_name);
    if (field.offset() != offset) {
        throw_typesystem_error("assert_field_offset({}, {}, {}) failed - got {}", type_name,
...
bool TypeSystem::try_lookup_method(const std::string &type_name, int method_id,
                                   MethodInfo *info) const {
    ...
    while (true) {
        ...
        if (iter_type->has_parent()) {
            iter_type = lookup_type(iter_type->parent());
        } else {
            break;
        }
    }
...
void TypeSystem::assert_method_id(const std::string &type_name, const std::string &method_name,
                                  int id) {
    auto info = lookup_method(type_name, method_name);
    if (info.id != id) {
        throw_typesystem_error(
            "Method ID assertion failed: type {}, method {} id was {}, expected {}", type_name,
...
bool TypeSystem::try_lookup_method(const std::string &type_name, const std::string &method_name,
                                   MethodInfo *info) const {
    auto kv = m_types.find(type_name);
    if (kv == m_types.end()) {
        // Try to look up a forward declared type
        auto fwd_dec_type = lookup_type_allow_partial_def(type_name);
        if (tc(TypeSpec("basic"), TypeSpec(fwd_dec_type->name()))) {
    ...
    if (field.is_array()) {
        ...
        if (field.is_inline()) {
            ...
            if (field_type->is_reference()) {
                ...
                return align(field_type->get_size_in_memory(),
                             field_type->get_in_memory_alignment());
            }
        }
    }
}
...
void TypeSystem::builtin_structure_inherit(StructureType *st) {
    st->inherit(get_type_of_type<StructureType>(st->parent()));
}
...
void find_field_access_paths(const TypeSystem *ts, const StructureType *type, int target_offset,
    ...
    for (const auto &field : type->fields()) {
        ...
        if (field_offset == target_offset && !field.is_array()) {
            ReverseLookupNode new_node{current_path,
                                       {FieldReverseLookupOutput::Token::Kind::FIELD, field.name(),
                                        -1, field.field_score()}};
        ...
        if (field.is_array() && !field.is_dynamic()) {
            if (target_offset >= field_offset && target_offset < field_offset + field_size) {
                ...
                ReverseLookupNode index_node{
                    &array_node,
                    {FieldReverseLookupOutput::Token::Kind::CONSTANT_IDX, "", index, 0.0}};
                    ...
                    if (auto element_struct = dynamic_cast<const StructureType *>(element_type)) {
                        if (Type::verbose)
                            fmt::print("DEBUG: Recursing into inline array element at offset {}\n",
                                       remainder);
                        find_field_access_paths(ts, element_struct, remainder, &index_node, results,
                                                depth + 1);
                    } else {
        ...
        if (field.is_inline() && !field.is_array()) {
            ...
            if (auto field_struct = dynamic_cast<const StructureType *>(field_type)) {
                if (target_offset >= field_offset && target_offset < field_offset + field_size) {
                    ...
                    find_field_access_paths(ts, field_struct, inner_offset, &inline_node, results,
                                            depth + 1);
                }
            }
        }
    ...
    if (type->has_parent()) {
        auto parent_type = ts->lookup_type_allow_partial_def(type->parent());
        if (auto parent_struct = dynamic_cast<const StructureType *>(parent_type)) {
            find_field_access_paths(ts, parent_struct, target_offset, current_path, results,
                                    depth + 1);
        }
    }
}
...
FieldReverseLookupOutput
TypeSystem::reverse_field_lookup(const FieldReverseLookupInput &input) const {
    ...
    if (auto structure = dynamic_cast<StructureType *>(base_type)) {
        if (Type::verbose) {
            fmt::print("DEBUG: Processing structure '{}' with {} fields, total size: {}\n",
                       structure->name(), structure->fields().size(),
                       structure->get_size_in_memory());

            // Выводим информацию о всех полях для отладки
            for (const auto &field : structure->fields()) {
                int field_size = get_size_in_type(field);
                fmt::print("DEBUG:   Field '{}' at offset {}, size: {}, inline: {}, array: {}, "
                           "array_size: {}\n",
                           field.name(), field.offset(), field_size, field.is_inline(),
                           field.is_array(), field.array_size());
            }
        }
            ...
            if (effective_offset >= field.offset() &&
                ...
                if (effective_offset == field.offset()) {
                    ...
                    result.tokens.push_back({
                        FieldReverseLookupOutput::Token::Kind::FIELD, field.name(), -1,
...
FieldReverseMultiLookupOutput
TypeSystem::reverse_field_multi_lookup(const FieldReverseLookupInput &input, int max_count) const {
