...
namespace script
{
    ...
    Object Reader::read_impl(std::shared_ptr<SourceText> source) {
        ...
        // Читаем все выражения без ожидания закрывающей скобки
        Object result = read_list(tokens, false);  // ← вот это важно!

        return Object::make_pair(
            Object::make_symbol(&m_symbols, "top-level"),
            result
        );
    }
    ...
    Object Reader::read_list(TokenStream& tokens, bool expect_close_paren) {
        ...
        while (tokens.has_more()) {
            ...
            if (!tokens.has_more()) {
                if (expect_close_paren) {
                    ...
                    break;  // конец файла - нормально
                }
            }

            if (expect_close_paren && tokens.peek() == ')') {
                tokens.read(); // consume ")"
                break;
            }
