...
class ReplWrapper {
  public:
    ...
    script::Interpreter &get_interpreter() {
        return interpreter_;
    }

    // Работа в многострочном режиме
    void set_multi_line_enabled(bool enabled) {
        multi_line_enabled_ = enabled;
    }
    void set_check_completion(bool check) {
        check_completion_ = check;
    }
  ...
  private:
    ...
    replxx::Replxx::hints_t       get_hints(const std::string &input, int &context_len,
