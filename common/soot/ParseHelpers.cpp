...
bool va_check(
    const Arguments &args, const std::vector<std::optional<ObjectType>> &unnamed,
    const std::unordered_map<std::string, std::pair<bool, std::optional<ObjectType>>> &named,
    std::string                                                                       *err_string) {
    ASSERT(!args.has_rest());
    if (unnamed.size() != args.unnamed.size()) {
        *err_string =
            fmt::format("Got {} arguments, but expected {}", std::to_string(args.unnamed.size()),
