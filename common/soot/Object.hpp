...
using BuiltinFormMethod = Object (Interpreter::*)(const Object &form, Arguments &args,
                                                  const std::shared_ptr<EnvironmentObject> &env);
using CustomFormMethod = std::function<Object(const Object& form, Arguments& args,
                                               const std::shared_ptr<EnvironmentObject>& env)>;
...
struct SpecialFormObject : public CallableObject {
    ...
    SpecialFormObject(SpecialFormMethod m, ArgumentSpec *s, std::string name)
     ...
     ~SpecialFormObject() override = default;
...
struct BuiltinFunctionObject : public CallableObject {
    ...
    BuiltinFunctionObject(BuiltinFormMethod m, ArgumentSpec *s, std::string name)
     ...
     ~BuiltinFunctionObject() override = default;
...
struct CustomFunctionObject : public CallableObject {
    CustomFormMethod method;
    ArgumentSpec     specs;
    std::string      name;

    CustomFunctionObject(CustomFormMethod m, ArgumentSpec *s, std::string name)
        : method(m), specs(true, false), name(name) {
        if (s)
            specs = *s;
    }
    ~CustomFunctionObject() override = default;
...
namespace std {
template <> struct hash<soot::InternedSymbolPtr> {
    size_t operator()(const soot::InternedSymbolPtr &s) const noexcept {
