...
void ReplWrapper::print_help() {
    ...
    fmt::print("  startup-post.gc      {} Run after network\n", "→");
...
replxx::Replxx::hints_t ReplWrapper::get_hints(
    ...
    if (input == "(help" || input == "help") hint_text = " - show help";
    else if (input == "(keybinds" || input == "keybinds") hint_text = " - show key bindings";
    else if (input == "(clear" || input == "clear") hint_text = " - clear screen";
    else if (input == "(quit" || input == "quit") hint_text = " - exit REPL";
    else if (input == "(history" || input == "history") hint_text = " - show command history";
    else return hints;
...
KeyBind::Modifier ReplWrapper::parse_modifier(const std::string& mod_str) {
    if (mod_str == "ctrl") return KeyBind::Modifier::CTRL;
