...
namespace vm {
    ...
    #define SC_ARG(arg_num, T, default_val) \
        vm::sc_arg_convert<T>(arg_num, argc, argv, SID(#T), (default_val))
    ...
    T sc_arg_convert(u32 arg_num, u32 argc, const Variant* argv,
        ...
        // јвтоматические преобразовани€ дл€ скал€ров
        ...
        // ƒл€ указателей - безопасное преобразование
        else if constexpr (std::is_pointer_v<T>) {
            if (actual_type == expected_type) {
                return static_cast<T>(arg.get_ptr());
            }
        }
        // ƒл€ других типов (структур и т.д.)
        else {
            if (actual_type == expected_type) {
                // «десь нужно безопасное преобразование
                return arg.get_ptr();
            }
        }
