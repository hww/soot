...
#include "common/util/Compress.hpp"
...
void GlobalProfiler::dump_to_json() {
...
#ifdef WITH_FILTER
        auto it = info_per_thread.find(event.tid);
        if (it == info_per_thread.end()) {
            continue; // пропускаем события из потоков без ROOT
        }
        auto &info = it->second;
    ...
    if (m_enable_compression) {
        ...
        auto file_path =
            file_util::get_path(file_util::PathType::PROJECT) / "profile_data" /
            fmt::format("prof-{}.json.zst", str_util::current_local_timestamp_no_colons());
        file_util::create_dirs_for_file(file_path);
        // file_util::write_binary(file_path, compressed_data.data(), compressed_data.size());
    } else {
        auto file_path = file_util::get_path(file_util::PathType::PROJECT) / "profile_data" /
                         fmt::format("prof-{}.json", str_util::current_local_timestamp_no_colons());
        file_util::create_dirs_for_file(file_path);
        file_util::write_text(file_path, json.dump());
    }
}
